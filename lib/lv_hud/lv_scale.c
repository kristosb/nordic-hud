...
static void scale_get_label_coords(lv_obj_t * obj, lv_draw_label_dsc_t * label_dsc, const char * text, lv_point_t * tick_point,
...
static void scale_build_custom_label_text(lv_obj_t * obj, lv_draw_label_dsc_t * label_dsc, const char * text,
                             ...
                             const uint32_t tick_idx)
{
    ...
    if(scale->txt_src) {
        scale_build_custom_label_text(obj, label_dsc, text_buffer, major_tick_idx);
    }
    ...
    if((LV_SCALE_MODE_VERTICAL_LEFT == scale->mode || LV_SCALE_MODE_VERTICAL_RIGHT == scale->mode)
       || (LV_SCALE_MODE_HORIZONTAL_BOTTOM == scale->mode || LV_SCALE_MODE_HORIZONTAL_TOP == scale->mode)) {
        scale_get_label_coords(obj, label_dsc,text_buffer, tick_point_b, &label_coords);
    }
        ...
        uint32_t label_gap = LV_SCALE_DEFAULT_LABEL_GAP; 
        ...
        scale_get_label_coords(obj, label_dsc,text_buffer, &point, &label_coords);
    }
    // Invalid mode 
    else {
        return;
                                          ...
                                          const uint16_t major_tick_idx)
{
    ...
    if(major_tick_idx <= scale->custom_label_cnt) {
        if(scale->txt_src[major_tick_idx - 1U]) {
            ...
            text = scale->txt_src[major_tick_idx - 1U];
            //label_dsc->text_local = 0;
        }
        else {
            text = NULL;
